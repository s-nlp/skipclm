...
def prepare_dataset(parallel_data, tokenizer):
    return datasets.Dataset.from_list([
        ...
        tokenizer(
            ...
            truncation=True,
            max_length=128
        )['input_ids'].squeeze().shape[0] <= 256
    ])

# Load the test data
def load_sentence_pairs(tokenizer, lang, reverse=False):
    ...
    if reverse:
        parallel_data = [{'sent1': eng, 'sent0': rus} for eng, rus in zip(eng_lines, rus_lines)]
    else:
        parallel_data = [{'sent0': eng, 'sent1': rus} for eng, rus in zip(eng_lines, rus_lines)]
    dataset = prepare_dataset(parallel_data, tokenizer)
    if reverse:
        sentence_pairs = [(f'{lang_code_to_name[lang]}: ' + item['source'], 'English: ' + item['target']) for item in dataset]
    else:
...
def load_model_and_tokenizer(model_name):
    if 'noskip' not in model_name and not 'nocontrastive' in model_name and model_name != 'facebook/xglm-564M':
        skip_start, skip_end = model_name.split('_')[4:6]
...
# Generate translations (Batched)
def generate_translations(model, tokenizer, sources, batch_size=128):
    translations = []
    for i in tqdm.tqdm(range(0, len(sources), batch_size)):
        batch_sources = sources[i:i + batch_size]
        inputs = tokenizer(batch_sources, return_tensors="pt", truncation=True, padding=True).to(model.device)
        with torch.no_grad():
            output_ids = model.generate(**inputs, max_new_tokens=128, eos_token_id=tokenizer.pad_token_id, temperature=0.0)
        batch_translations = tokenizer.batch_decode(
            output_ids[:, inputs['input_ids'].shape[1]:], # Slice based on batch input length
...
def calculate_bleu_score(generated_translations, references, lang='non_zh'):
    if lang == 'zh':
...
def calculate_scores():
    ...
    parser.add_argument("--batch_size", type=int, default=128, help="Batch size for translation generation.") # Added batch_size argument
    ...
    if not args.noinference:
        ...
        # Load data
        sentence_pairs = load_sentence_pairs(tokenizer, args.lang, args.reverse_direction)
        sources, references = zip(*sentence_pairs)

        # Generate translations (Batched)
        generated_translations = generate_translations(model, tokenizer, sources, batch_size=args.batch_size) # Pass batch_size
        ...
        df = pd.read_csv(args.output_file.split('.')[0] + '.csv', sep='\t')
        sources, references, generated_translations = [], [], []
        for a in df.iterrows():
            src, ref, tr = a[1]
            sources.append(src)
            references.append(ref)
            generated_translations.append(tr)
    ...
    with open(args.output_file.split('.')[0] + f'_reverse_{args.reverse_direction}' + '.json', 'w') as f:
        json.dump(results, f, indent=4)

    print(args.output_file.split('.')[0] + f'_reverse_{args.reverse_direction}' + '.' + args.output_file.split('.')[1])
    print(args.output_file.split('.')[0] + f'_reverse_{args.reverse_direction}' + '.csv')
    df.to_csv(args.output_file.split('.')[0] + f'_reverse_{args.reverse_direction}' + '.csv', sep='\t')
    df, comet_score = eval_comet.calculate_comet_scores(args.output_file.split('.')[0] + f'_reverse_{args.reverse_direction}' + '.csv', 'Unbabel/wmt22-comet-da', reverse=args.reverse_direction)
    results['COMET'] = comet_score

    with open(args.output_file, 'w') as f:
        json.dump(results, f, indent=2)

    df.to_csv(args.output_file.split('.')[0] + f'reverse_{args.reverse_direction}' + '_comet' + '.csv', index=False)

    print(args.output_file)
    print(json.dumps(results, indent=2))

    with open(args.output_file.split('.')[0] + f'reverse_{args.reverse_direction}' + '.csv', 'w') as f:
        f.write('\n'.join([f'{src if not src.endswith("\n") else src[:-1]}\t{ref}\t{tr}' for src, ref, tr in zip(sources, generated_translations, references)]))
